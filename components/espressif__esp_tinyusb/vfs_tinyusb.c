...
static ssize_t tusb_read(int fd, void *data, size_t size)
{
    ...
    while (received < size) {
        ...
        uint8_t tmpc = 0;
        tinyusb_cdcacm_read(s_vfstusb.cdc_intf, &tmpc, 1, &rx_size);
        c = rx_size ? (int) tmpc: NONE;
...
static int tusb_fcntl(int fd, int cmd, int arg)
{
    ...
    case F_SETFL:
        ...
        if (blocking) {
            result = tusb_cdc_enable_rx_blocking(s_vfstusb.cdc_intf);
        } else {
            result = tusb_cdc_disable_rx_blocking(s_vfstusb.cdc_intf);
        }
        s_vfstusb.flags = arg;
        break;
    default:

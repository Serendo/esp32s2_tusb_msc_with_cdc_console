...
int tusb_cdc_enable_rx_blocking(tinyusb_cdcacm_itf_t itf)
{
    ...
    esp_err_t err = tinyusb_cdcacm_register_callback(itf, CDC_EVENT_RX, &blocking_rx_cb);
    if (err != ESP_OK) {
        errno = ENODEV;
        goto fail;
    }
...
int tusb_cdc_disable_rx_blocking(tinyusb_cdcacm_itf_t itf)
{
    tinyusb_cdcacm_register_callback(itf, CDC_EVENT_RX, NULL);
    if (s_rx_semaphore) {
        vSemaphoreDelete(s_rx_semaphore);
        s_rx_semaphore = NULL;
    }
